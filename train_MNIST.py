...
from datasets import get_mnist_datasets
...
def main(args):
    ...
    # Load datasets
    train_loader, val_loader = get_mnist_datasets(batch_size)

    # Instantiate the model
    model = VAE_MNIST(latent_dim).to(args.device)

    # Loss function
    # Reconstruction loss + KL divergence
    criterion = nn.BCELoss(reduction="sum")
    def loss_function(recon_x, x, mu, log_var):
        recon_loss = criterion(recon_x, x.view(-1, 28 * 28))
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            save_model(model, os.path.join(checkpoint_dir, "vae_mnist_best.pth"))
    ...
    dummy_input = torch.randn(1, 28, 28, device=args.device)
